...
function ScrollTop(){
  ...
  useEffect(()=> { try { window.scrollTo(0,0); } catch(e) {} }, [pathname]);
  return null;
}
...
function Shell(){
  ...
  const nav = [['Home','/'],['Products','/products'],['How We Think','/thinking'],['Lab','/lab'],['About','/about'],['Contact','/contact']];
  return <>
  ...
  return <>
    <section className="hero section-shell">
      <div className="hero-copy">
        ...
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:.24}} className="actions"><NavLink className="primary" to="/products">Explore Products <ArrowRight size={18}/></NavLink><NavLink className="ghost" to="/thinking">How Pilinix Thinks</NavLink></motion.div>
      </div>
      <HeroGraph/>
    ...
    <section className="section-shell think-preview">
      ...
      <FlowStrip/>
      <div className="center-action"><NavLink className="ghost" to="/thinking">Explore the model <ArrowRight size={16}/></NavLink></div>
    </section>
...
function ProductCard({p,i}){ const Icon=p.icon; return <motion.article initial={{opacity:0,y:22}} whileInView={{opacity:1,y:0}} viewport={{once:true,amount:.2}} transition={{delay:i*.07}} whileHover={{y:-8}} className="product-card" style={{'--accent':p.accent}}>
  ...
  <NavLink className="text-link" to={`/products/${p.slug}`}>Explore {p.name} <ArrowRight size={15}/></NavLink>
